...
# --- Streamlit Setup ---
st.set_page_config(page_title="Content Aggregation", page_icon="🔗", layout="wide")

# --- Login Page ---
def login():
    st.title("🔐 CONTENT AGGREGATION")
...
def main_app():
    ...
    if query:
        with st.spinner("Getting general info and fetching news..."):
            topic_summary = get_article_theory(query)
        ...
        if articles:
            ...
            # Create a relevance score chart using Altair
            st.subheader("📊 Relevance Score Chart")
            df = pd.DataFrame(articles)
            chart = alt.Chart(df).mark_bar().encode(
                x=alt.X("title:N", sort=None, axis=alt.Axis(labelFontSize=18, titleFontSize=22)),  # Customize font sizes
                y=alt.Y("relevance_score:Q", axis=alt.Axis(labelFontSize=18, titleFontSize=22)),
                tooltip=["title", "relevance_score"]
            ).properties(
                width=800,
                height=500
            )# ).configure_axis(
